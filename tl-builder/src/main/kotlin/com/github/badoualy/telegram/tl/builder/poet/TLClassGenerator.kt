...
class TLClassGenerator(tlDefinition: TLDefinition, val config: Config) {
        ...
        TypeSpec.objectBuilder(name)
            ...
            .addFunction(FunSpec.makeOverride("registerClasses").apply {
                constructors
                    ...
                    .forEach {
                        addStatement(
                            "registerClass(%T.CONSTRUCTOR_ID, %T::class.java)",
                            it, it
                        )
                    }
            }.build())
    ...
    private fun TLConstructor.generateClass(): TypeSpec {
        val clazz = TypeSpec.makeTLClass(
            tlClassName(),
        ...
        when (tlType) {
            ...
            is TLTypeGeneric -> {
                ...
                responseFun.addStatement(
                    "return tlDeserializer.$methodName()",
    ...
    fun TLTypeConstructor<*>.generateClassCommon(clazz: TypeSpec.Builder) {
        ...
        if (condParameters.isNotEmpty() && id != null) {
            ...
            computeFlagsFun.addStatement("_flags2 = 0")
                ...
                if (realType.isTrueFalseFlag()) {
                    ...
                    if (realType is TLTypeRaw && realType.name == "Bool" && condParameters.any { it != parameter && (it.tlType as ConditionalType).value == tlType.value }) {
                        ...
                        computeFlagsFun.addStatement(
                            "if ($fieldName != null && !isMask(${tlType.version}, ${tlType.pow2Value()})) $fieldName = null"
                        )
                    } else {
                        computeFlagsFun.addStatement(
            ...
            if (condBoolean.isNotEmpty()) {
                ...
                for (parameter in condBoolean) {
                    val conditionalType = parameter.tlType as ConditionalType
                    computeFlagsFun.addStatement(
                        "" + parameter.name.lCamelCase()
                            .javaEscape() + " = isMask(${conditionalType.version}, ${conditionalType.pow2Value()})"
                    )
                }
            }
...
//            println("Log this: ${parameter.name.lCamelCase().javaEscape()}")
            ...
            if ((fieldTlType !is TLTypeFlag && fieldTlType !is TLTypeFlag2) && (id != null || parameter.inherited)) {
                clazz.addProperty(PropertySpec.varBuilder(fieldName, fieldType)
                    .apply {
                        if (id == null) {
                            addModifiers(KModifier.ABSTRACT)
                        } else {
                            if (parameter.inherited) addModifiers(KModifier.OVERRIDE)
                            getInitValue(fieldType).let {
                                initializer(
                                    it,
                                    if (it.contains("%T", true)) fieldTlType.findDefaultConstructor()
                                    else Unit
                                )
                            }
                        }
                        ...
                        if ((fieldTlType is TLTypeFlag || fieldTlType is TLTypeFlag2) && (id == null || !parameter.inherited))
                            addModifiers(
                                if (id == null) KModifier.PROTECTED
            ...
            if (fieldTlType.serializable()) {
                ...
                computeSizeFun.addStatement(
                    "size += " + computeSizeParameter(
                        fieldName,
                        fieldTlType
                    )
                )
            ...
            if (fieldTlType !is TLTypeFlag && fieldTlType !is TLTypeFlag2) {
        ...
        if (id != null) {
            ...
            // _constructor
            clazz.addProperty(
                PropertySpec.builder("_constructor", String::class)
    ...
    private fun TLMethod.makeApiWrapperFun(responseType: TypeName, methodName: String, addExceptions: Boolean) =
        FunSpec.makeOverride(name.lCamelCase())
            ...
            .addStatement("return $methodName(%T(%L))",
                ...
                if (parameters.isNotEmpty()) {
                    parameters
                        .filterNot { it.tlType is TLTypeFlag || it.tlType is TLTypeFlag2 }
                        .joinToString(
                            ", "
