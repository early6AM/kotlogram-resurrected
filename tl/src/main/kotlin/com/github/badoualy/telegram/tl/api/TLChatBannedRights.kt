...
class TLChatBannedRights() : TLObject() {
    ...
    override fun deserializeBody(tlDeserializer: TLDeserializer) = with (tlDeserializer)  {
        _flags = readInt()
        viewMessages = isMask(1, 1)
        sendMessages = isMask(1, 2)
        sendMedia = isMask(1, 4)
        sendStickers = isMask(1, 8)
        sendGifs = isMask(1, 16)
        sendGames = isMask(1, 32)
        sendInline = isMask(1, 64)
        embedLinks = isMask(1, 128)
        sendPolls = isMask(1, 256)
        changeInfo = isMask(1, 1024)
        inviteUsers = isMask(1, 32768)
        pinMessages = isMask(1, 131072)
        manageTopics = isMask(1, 262144)
        sendPhotos = isMask(1, 524288)
        sendVideos = isMask(1, 1048576)
        sendRoundvideos = isMask(1, 2097152)
        sendAudios = isMask(1, 4194304)
        sendVoices = isMask(1, 8388608)
        sendDocs = isMask(1, 16777216)
        sendPlain = isMask(1, 33554432)
        untilDate = readInt()
    }
